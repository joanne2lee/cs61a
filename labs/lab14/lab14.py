...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if self.holder == check.payable_to and check.deposited == False:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            e, c = helper_align(skeleton_idx + 1, code_idx + 1)
            s = code_char + e
            possibilities.append((s, c))
        # Add
        e, c = helper_align(skeleton_idx, code_idx + 1)
        s = "+[" + code_char + "]" + e
        possibilities.append((s, c+1))
        # Delete
        e, c = helper_align(skeleton_idx + 1, code_idx)
        s = "-[" + skel_char + "]" + e
        possibilities.append((s, c+1))
...
def filterl(lst, pred):
    ...
    Link(4, Link(2))
    """
    def link_f(first,f_rest):
        if pred(first):
            return Link(first, f_rest)
        return f_rest
    return foldr(lst, link_f, Link.empty)
