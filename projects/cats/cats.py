...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    paragraphs_to_choose_from = [x for x in paragraphs if select(x) is True]
    if k+1 > len(paragraphs_to_choose_from):
        return ''
    else:
        return paragraphs_to_choose_from[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def contain(paragraph):
        ...
        if any(i in words for i in topic):
            return True
        else:
            return False
    return contain
...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        correct_words -= (len(typed_words)-len(reference_words))
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        closest_valid = min(valid_words,key=lambda x: diff_function(user_word,x,limit))
        if diff_function(user_word,closest_valid,limit) <= limit:
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            user_progress = correct / len(prompt)
...
def time_per_word(times_per_player, words):
    ...
    for n in range(len(times_per_player)):
        players = []
        for i in range(len(times_per_player[n])-1):
            t = times_per_player[n][i+1] - times_per_player[n][i]
            players.append(t)
        times.append(players)
...
def fastest_words(game):
    ...
    for p in player_indices:
        ...
        for w in word_indices:
            fastest_player = min(player_indices,key=lambda p:(time(game,p,w)))
            if p == fastest_player:
                player.append(word_at(game,w))
        player_words.append(player)
    return player_words
    # END PROBLEM 10
